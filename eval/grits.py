...
def cells_to_grid(cells, key='bbox'):
    ...
    for row_num in range(num_rows):
        for column_num in range(num_columns):
            if cell_grid[row_num][column_num] == 0: # incomplete reconstructure
                if key == 'bbox':
                    cell_grid[row_num][column_num] = [0.0,0.0,1.0,1.0] # invalid cell
                else:
                    cell_grid[row_num][column_num] = 'invalid_cell'
...
def cells_to_relspan_grid(cells):
    ...
    """
    ## TODO: incomplete table structure..
    if len(cells) == 0:
        return [[]]
    num_rows = max([max(cell['row_nums']) for cell in cells])+1
    num_columns = max([max(cell['column_nums']) for cell in cells])+1
    ...
    for row_num in range(num_rows):
        for column_num in range(num_columns):
            if cell_grid[row_num][column_num] == 0: # incomplete reconstructure
                cell_grid[row_num][column_num] = [0,0,1,1]
    return cell_grid 
...
def cell_to_dilatedbbox_grid(cells):
    """
    Compute the matrix of grid cell features for GriTS_Loc but using the raw predicted
    and ground truth bounding boxes, not the post-processed boxes.
    ...
    *dilated*, which means they are larger than the actual ground truth boxes.
    ...
    an individual model.
    """
    # gather same row/col cells
    rows_item = {}
    cols_item = {}
    for c in cells:
        for r in c['row_nums']:
            rows_item[r] = rows_item.get(r, []) + [c['bbox']]
        for cl in c['column_nums']:
            cols_item[cl] = cols_item.get(cl, []) + [c['bbox']]
    
    # get row/column objects
    for k, v in rows_item.items():
        # bbox: [x0, y0, x1, y1]
        bbox = np.asarray(v)
        x0, y0, x1, y1 = bbox[:, 0].min(), bbox[:, 1].max(), bbox[:, 2].max(), bbox[:, 3].min()
        rows_item[k] = [x0, y0, x1, y1]
    
    for k, v in cols_item.items():
        bbox = np.asarray(v)
        x0, y0, x1, y1 = bbox[:, 0].max(), bbox[:, 1].min(), bbox[:, 2].min(), bbox[:, 3].max()
        cols_item[k] = [x0, y0, x1, y1]

    rows = [{'bbox': bbox} for bbox in rows_item.values()]
    columns = [{'bbox': bbox} for bbox in cols_item.values()]
    rows.sort(key=lambda x: x['bbox'][1]+x['bbox'][3])
    columns.sort(key=lambda x: x['bbox'][0]+x['bbox'][2])

    # construct cell_grid
    cell_grid = [[[] for _ in range(len(columns))] for _ in range(len(rows))]
