...
class CocoDetection(coco_base.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        # self.check_annotation(img, target, image_id, 7) #CLASS_OFFSET)
        return img, target

    def check_annotation(self, img, target, image_id, CLASS_OFFSET):
        bbox = target['boxes']
        ...
        heatmap.save(f'./check/heatmap_{image_id}_{name}')
...
class ConvertCocoPolysToMask(object):
    def __init__(self, 
        ...
        # if vocab_file is not None: # include OCR # TODO
        #     from .tokenizer import S2TabTokenizer
        #     self.tokenizer = S2TabTokenizer(None, 
        #                                     vocab_file,
        #                                     None,
        #                                     vocab['BOS'],
        #                                     vocab['EOS'],
    ...
    def __call__(self, image, _target):
        ...
        boxes = [obj['bbox'] for obj in anno] # x1,y1,w,h
        ...
        if 'text_bbox' in anno[0]:
            text_boxes = [(len(obj['text_bbox']) > 0) * obj['text_bbox'] + 
                            (len(obj['text_bbox']) == 0) * obj['bbox'] for obj in anno]
        ...
        def filter_tokens(t):
            # make sequence, add '[empty]' to empty tokens
            if t == []:
                return ['[empty]','[eos]'] # '[bos]',
            else:
                return t + ['[eos]'] # ['[bos]'] + 
        ...
        if 'tokens' in anno[0]:
            if self.tokenizer is not None:                
                content_list = [self.tokenizer.get_labels(obj['tokens'], padding=True, case_sensitive=True, append_eos=True, filter=True) for obj in anno]
                # content_list = [[self.tokenizer.char_to_label[x] for x in filter_tokens(obj['tokens'])] for obj in anno]
                content_list = [torch.tensor(text) for text in content_list]
                content_list = [text for text, keep_instance in zip(content_list, keep) if keep_instance]
            
                # make content as a tensor of (num_cell, token_max_length)
                content = torch.zeros((len(content_list),
                                       self.tokenizer.max_length),
                                      dtype=content_list[0].dtype,
                                      device=content_list[0].device) + \
                          self.tokenizer.char_to_label['[pad]']
                for i, c in enumerate(content_list):
                    c_len = min(len(c), self.tokenizer.max_length)
                    content[i, :c_len] = c[:c_len]

            check_content = np.asarray([''.join(obj['tokens']) for obj in anno])
            check_content = check_content[keep]
            is_empty = torch.as_tensor([(len(x) > 0) * False + (len(x) == 0) * True for x in check_content])
        else:
            content = None
            is_empty = torch.as_tensor([False for _ in range(classes.shape[0])])
        ...
        if 'is_header' in anno[0]:
            ...
            header_classes = header_classes[keep] #header_classes[idx][keep]
        else:
            header_classes = None
        ...
        if row_target != []:
            row_boxes = torch.stack([torch.as_tensor(obj) for obj in row_target], dim=0)
            col_boxes = torch.stack([torch.as_tensor(obj) for obj in col_target], dim=0)
        
            th = torch.stack([col_boxes[structure[:, 0]], row_boxes[structure[:, 2]], 
                        col_boxes[structure[:, 1]], row_boxes[structure[:, 3]]], dim=1) # th for x1,y1,x2,y2 (bs, 4, 2)
            th = th - boxes[..., None]
        ...
        if self.include_heatmap:
            ...
            for e, b in zip(is_empty, text_boxes.clone()):
                if not self.include_empty_cells and e:
                    continue
                b = list(map(int, b))
        ...
        target = {}
        target["boxes"] = text_boxes
        target["original_boxes"] = copy.deepcopy(boxes)
        target["labels"] = classes
        target["endrow"] = endrow
        target["image_id"] = image_id
        target["table_box"] = torch.as_tensor([tx1,tx2-tx1,ty1,ty2-ty1], dtype=torch.float32)
        target["structure"] = structure
        target["is_empty"] = torch.as_tensor(is_empty)
        if self.tokenizer is not None:
            target["content"] = content
            target["pad_idx"] = self.tokenizer.char_to_label['[pad]']
        if row_target != []:
            target["structure_rc"] = structure_by_rc # (rowcol_num, cell_max_num, 4) # cell_max_num may have dummy cell
...
def make_coco_transforms(image_set, max_seq_len=100, 
    ...
    normalize = T.Compose([
        T.ToTensor(),
        T.Normalize([0.909, 0.919, 0.915], [0.203, 0.152 , 0.187]) # PTN, STN, FTN
    ])
    ...
    if image_set == 'train':
        ...
        transforms += [normalize,
                       T.ConstructSeq(max_seq_len, loss_eos_token_weight, seq_version, vocab)]
    ...
    if image_set == 'val' or image_set == 'test':
        return T.Compose([
            ...
            normalize,
            T.ConstructSeq(max_seq_len, 0., seq_version, vocab),
        ])
