# -----------------------------------------------------------------------
# S2Tab official code : models/transformer_decoder_mult_ver2_1_1_empty.py
# -----------------------------------------------------------------------
...
class BertMultiEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        elif config.position_embedding_type == 'sine' or 'sine' in config.position_embedding_type:
            self.position_embeddings = PositionalEmbedding(config.hidden_size)
        elif config.position_embedding_type == 'none' or 'relative' in config.position_embedding_type:
    ...
    def forward(
        ...
        elif self.position_embedding_type == "sine" or "sine" in self.position_embedding_type:
...
class BertDetrSelfAttention(BertSelfAttention):
    def __init__(self, config, position_embedding_type=None, cross_modal=False):
        ...
        if 'relative' in self.position_embedding_type:
            self.relative_attention_num_buckets = 512 # train max dist
            self.relative_attention_max_distance = config.max_position_embeddings
    ...
    def compute_bias(self, query_length, key_length, device=None):
        """Compute binned relative position bias - t5"""
        if device is None:
        ...
        relative_position_bucket = self._relative_position_bucket(
            ...
            bidirectional=False, #(not self.is_decoder), 
    ...
    def forward(
        ...
        if "relative" in self.position_embedding_type:
            real_seq_length = query_layer.shape[2]
            if past_key_value is not None:
                assert (
                    len(past_key_value) == 2
                ), f"past_key_value should have 2 past states: keys and values. Got { len(past_key_value)} past states"
                real_seq_length += past_key_value[0].shape[2]
            key_length = real_seq_length if key_layer is None else key_layer.shape[2]
            position_bias = self.compute_bias(real_seq_length, key_length, device=attention_scores.device)
...
class TableDecoder(BertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=False):
        ...
        self.empty_box_head = MLP(in_dim_box, config.hidden_size, 1, 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        is_empty = self.empty_box_head(box_sequence_output)
