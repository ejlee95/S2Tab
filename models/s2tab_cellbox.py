# -----------------------------------------------------------------------
# S2Tab official code : models/network_box_regression_mult_ver2_1_1.py
# -----------------------------------------------------------------------
...
from .transformer_cellbox import build_transformer
...
class Tab2seq(nn.Module):
    ...
    def forward(self, samples: NestedTensor, sources: dict = None, 
                       draw_attention: bool = False, **kwargs):
        """
        ...
        if sources is not None:
            ...
            return self.infer(samples, draw_attention)
    ...
    def train_step(self, samples: NestedTensor, sources: dict = None):
        ...
        src_type_row = torch.where(src_type > CLASS_OFFSET, 
                                      (src_type - 1 - CLASS_OFFSET) % MAX_COL + \
                                       1 + CLASS_OFFSET, 
                                      src_type)
        # src_type_row = torch.where(src_type > CLASS_OFFSET+ENDROW_OFFSET, 
        #                             (src_type - 1 - CLASS_OFFSET-ENDROW_OFFSET) % MAX_COL + \
        ...
        src_type_col = torch.where(src_type > CLASS_OFFSET, 
                                        ...
                                        1 + CLASS_OFFSET, 
                                      src_type)
        # src_type_col = torch.where(src_type > CLASS_OFFSET+ENDROW_OFFSET, 
        #                                 torch.div(src_type - 1 - CLASS_OFFSET - ENDROW_OFFSET, 
        #                                           MAX_COL, rounding_mode='floor') + \
        #                                 1 + CLASS_OFFSET + MAX_COL, 
        #                             src_type_col)
    ...
    def infer(self, samples: NestedTensor, draw_attention: bool=False):
        """
        - samples.tensor: batched images [N x 3 x H x W]
        - samples.mask: a binary mask of [N x H x W], containing 1 on real pixels
...
class SeqCriterion(nn.Module):
    ...
    def loss_aux_heatmap_regression(self, outputs, targets, **kwargs):
        ...
        # loss = F.mse_loss(src_heatmap, tgt_heatmap, reduction='none')
        loss = heatmap_focal_loss(src_heatmap, tgt_heatmap, gamma=2, beta=4, reduction='none')
...
class Postprocessor(nn.Module):
    ...
    def __init__(self, enc_config=None, dec_config=None, vocab=None):
        super().__init__()
    ...
    def forward(self, outputs, target_sizes, pp=False):
        ...
        logits_row = outputs['pred_type_logits_row'] #.decompose()[0] # (batch, num_boxes, num_classes)
        logits_col = outputs['pred_type_logits_col'] #.decompose()[0] # (batch, num_boxes, num_classes)
        ...
        for bi in range(bs):
            ...
            if (sum(lab_row == EOS) == 0 and sum(lab_row == PAD) == 0):
                ...
                end_idx_r = torch.argmin((lab_row != PAD).to(torch.float32)).item()
            else:
                end_idx_r = torch.argmin((lab_row != EOS).to(torch.float32)).item()

            if (sum(lab_col == EOS) == 0 and sum(lab_col == PAD) == 0):
                end_idx_c = lab_col.shape[0]
            elif sum(lab_col == EOS) == 0 and sum(lab_col == PAD) > 0:
                end_idx_c = torch.argmin((lab_col != PAD).to(torch.float32)).item()
            else:
                end_idx_c = torch.argmin((lab_col != EOS).to(torch.float32)).item()
        ...
        for box, lab, endrow_lab, score, structure, mask, all_box in \
            zip(valid_boxes, valid_labels, valid_endrow_labels, valid_scores, structures, valid_masks, all_boxes):
...
def build(args, vocab=None):
    ...
    model = Tab2seq(backbone, transformer, vocab=vocab)
    ...
    postprocessor = Postprocessor(transformer.encoder.config, transformer.decoder.config, vocab)
