# -----------------------------------------------------------------------
# S2Tab official code : models/generation_utils_multi_task_mult_ver2.py
# -----------------------------------------------------------------------
    ...
    **model_kwargs,
    ...
    while True:
        ...
        if self.config.eot_token_id is not None:
            eot_mask = torch.logical_or(next_tokens_row == self.config.eot_token_id, next_tokens_col == self.config.eot_token_id)
        ...
        if self.config.eot_token_id is not None:
            eot_mask = eot_mask.unsqueeze(1).unsqueeze(2).repeat((1,next_coords_ids.shape[1], next_coords_ids.shape[2]))
        ...
        if self.config.eot_token_id is not None:
            next_coords_ids = torch.where(eot_mask == True, torch.zeros_like(next_coords_ids) + self.config.eot_token_id, next_coords_ids)
